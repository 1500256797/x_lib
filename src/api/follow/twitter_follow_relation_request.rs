...
pub struct GetUserFollowersListRequest {
    pub user_id: String,
    pub csrf_token: String,
    pub bearer_token: String,
}

impl IntoRequestBuilder for GetUserFollowersListRequest {
    fn into_request(self, client: reqwest::Client) -> reqwest::RequestBuilder {
        let variables = json!(
            {"userId":self.user_id,"count":20,"includePromotedContent":false}
        );
        let features = json!(
